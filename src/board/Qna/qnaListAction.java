...
import board.Qna.qnaPagingAction;
...
public class qnaListAction extends ActionSupport{
	...
	public String search() throws Exception {
		...
		if(searchNum == 0){
			list = sqlMapper.queryForList("Qna-selectSearchW", "%"+getSearchKeyword()+"%");
		}
		if(searchNum == 1){
			list = sqlMapper.queryForList("Qna-selectSearchS", "%"+getSearchKeyword()+"%");
		}
		if(searchNum == 2){
			list = sqlMapper.queryForList("Qna-selectSearchC", "%"+getSearchKeyword()+"%");	
		}
